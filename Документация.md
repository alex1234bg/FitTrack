...
- Потребителят може **ръчно да добавя единична тренировка** към всеки ден от календара — само от програми от тип „Workout Only".
- Добавените ръчно тренировки могат да се премахват с бутон ✕ директно от панела на деня.
- Всички операции (добавяне, премахване, маркиране) работят без презареждане на страницата (AJAX).
...
- В колоната „Duration" програмите от тип „Workout Only" показват badge **„Workout"** вместо брой седмици.
- Броят упражнения се изчислява правилно според типа на програмата: за „Workout Only" се броят директните упражнения, а за програми със седмичен план — упражненията вътре в дните.
...
Проектът съдържа **11 автоматизирани теста** в отделен проект `FitTrack.Tests`.

### Използвани библиотеки

| Библиотека | Предназначение |
|---|---|
| **xUnit** | Тестов framework — дефинира и изпълнява тестовете |
| **Moq** | Създава „фалшиви" обекти (mock) за зависимости като `UserManager` |
| **EF Core InMemory** | Заменя реалната SQL база с база в паметта — тестовете не изискват SQL Server |

### Тестови класове и покрити случаи

#### HomeControllerTests (3 теста)
| Тест | Проверява |
|---|---|
| `Index_ReturnsViewResult` | `Index()` връща ViewResult |
| `Index_EmptyDatabase_TotalProgramsIsZero` | При празна база `TotalPrograms` е 0 |
| `Index_WithFivePrograms_FeaturesAtMostThree` | Препоръчаните програми са максимум 3 |

#### WorkoutProgramsControllerTests (4 теста)
| Тест | Проверява |
|---|---|
| `Index_ReturnsViewResult` | `Index()` връща ViewResult |
| `Index_FilterByLevel_ReturnsOnlyMatchingPrograms` | Филтърът по ниво работи правилно |
| `Details_WithValidId_ReturnsViewResult` | Валидно ID връща ViewResult |
| `Details_WithInvalidId_ReturnsNotFound` | Невалидно ID връща NotFound (404) |

#### UserProgramsControllerTests (4 теста)
| Тест | Проверява |
|---|---|
| `Enroll_WithInvalidProgramId_ReturnsNotFound` | Записване в несъществуваща програма връща NotFound |
| `Enroll_WithValidProgram_AddsUserProgramToDatabase` | Успешното записване добавя ред в `UserPrograms` |
| `Enroll_Twice_DoesNotDuplicateUserProgram` | Двойно записване не създава дублиран запис |
| `Unenroll_WhenEnrolled_RemovesUserProgram` | Отписването премахва записа от `UserPrograms` |
